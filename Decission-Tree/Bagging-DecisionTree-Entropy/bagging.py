...
def train(training_data, attr_names, label_name, num_attr, num_rounds):
    ...
    for n in range(num_rounds):
        resampled_entries = []
        random_indices = random.sample(list(range(0, num_rows)),num_rows)
...
def main():
    ...
    (attr_names, training_data) = decisiontree.read_data(train_filename, num_attr)
    (attr_names, test_data) = decisiontree.read_data(test_filename, num_attr)
    label_name = 'IsBadBuy'
