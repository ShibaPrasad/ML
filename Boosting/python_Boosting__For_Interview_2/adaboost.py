...
def train(data,label,numIt = 1000):
    ...
    for i in range(numIt):
        ...
        EnsembleErrors = multiply(sign(EnsembleClassEstimate)!=mat(label).T,ones((m,1)))  #Converte to float
        errorRate = EnsembleErrors.sum()/m
        print ("total error:  ",errorRate)
        if errorRate == 0.0:
            break
     # print weakClassifiers    # Erase Later
    return weakClassifiers

# Separated  for Clarity By Shiba
